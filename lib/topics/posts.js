...
module.exports = function (Topics) {
  ...
  Topics.addPostData = async function (postData, uid) {
    ...
    const [bookmarks, endorsed, endorsedBy, voteData, userData, editors, replies] = await Promise.all([posts.hasBookmarked(pids, uid), posts.hasEndorsed(pids), posts.endorsedBy(pids), posts.getVoteStatusByPostIDs(pids, uid), getPostUserData('uid', async uids => await posts.getUserInfoForPosts(uids, uid)), getPostUserData('editor', async uids => await user.getUsersFields(uids, ['uid', 'username', 'userslug'])), getPostReplies(pids, uid), Topics.addParentPosts(postData)]);
